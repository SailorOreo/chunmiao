...
WELCOME_SENTENCE = "欢迎使用基于yolov11和yolov8的钢铁缺陷检测系统"      # 欢迎的句子
...
class MainWindow(QTabWidget):
    def __init__(self):
        ...
        # 用来进行设置的参数
        self.init_vid_id = 'http://192.168.162.7:8090/stream.mjpg'  # 网络摄像头URL，替换为实际IP和端口
        self.vid_source = self.init_vid_id      # 直接使用URL字符串，而不是强制转换为整数
        ...
        self.model_path = "runs/yolo11s_pretrained/train/weights/best.pt"  # 模型路径
    ...
    def open_cam(self):
        ...
        # 如果是纯数字，说明是本地 UVC 摄像头，转成 int；否则保留字符串（URL/路径）
        if str(self.vid_source).isdigit():
            src = int(self.vid_source)
            api = cv2.CAP_ANY
        else:
            src = str(self.vid_source)
            # 只要是 http/https/rtsp，都强制走 FFmpeg，避免被 CAP_IMAGES 误判
        ...
        # 打不开时，做一次 URL 兜底：把 .mjpg 换成 ?action=stream 再试
        if not self.cap.isOpened() and isinstance(src, str) and src.endswith(".mjpg"):
            alt = src.replace("/stream.mjpg", "/?action=stream")
            print(f"主 URL 打不开，尝试备用：{alt}")
            self.cap = cv2.VideoCapture(alt, cv2.CAP_FFMPEG)
            if self.cap.isOpened():
                self.vid_source = alt  # 记住可用的地址

        if not self.cap.isOpened():
            QMessageBox.warning(self, "连接失败", "无法打开摄像头源，请检查URL或网络")
            self.reset_vid()
            return
    ...
    def detect_vid(self):
        ...
        while self.cap.isOpened():
            ...
            if success:
                # 处理检测逻辑
                results = self.model(frame, conf=self.conf_thres, iou=self.iou_thres, save_txt=self.save_txt,
                                     save_conf=self.save_conf, save_crop=self.save_crop)
                result = results[0]  # 获取检测结果
                img_array = result.plot()  # 在图像上绘制检测结果
                im0 = img_array
                im_record = copy.deepcopy(im0)
    ...
    def save_config_change(self):
        ...
        try:
            ...
            self.vid_source = str(self.config_vid_source_value.text())  # 从配置框获取视频源
