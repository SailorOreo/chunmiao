...
# todo 修改为你的数据的根目录
FILE_ROOT = "F:/Upppppdate/37-/"
IMAGE_SET_ROOT = FILE_ROOT + "VOC2007-defect/Main"   # 图片区分文件的路径
IMAGE_PATH = FILE_ROOT + "VOC2007-defect/JPEGImages"  # 图片的位置
ANNOTATIONS_PATH = FILE_ROOT + "VOC2007-defect/Annotations"  # 数据集标签文件的位置
LABELS_ROOT = FILE_ROOT + "VOC2007-defect/Labels"  # 进行归一化之后的标签位置
DEST_PPP = FILE_ROOT + "elec_defect_yolo_format"
DEST_IMAGES_PATH = "elec_defect_yolo_format/images"  # 区分训练集、测试集、验证集的图片目标路径
DEST_LABELS_PATH = "elec_defect_yolo_format/labels"  # 区分训练集、测试集、验证集的标签文件目标路径
...
if __name__ == '__main__':
    ...
    for file in files_all:
        if file.split(".")[-1] == "xml":
            files.append(file.split(".xm")[0])
    # print(len(files))
    for file in files:
        # print("file name: ", file)
        file_xml = file.split(".xm")
        # try:
        print(file_xml[0])
